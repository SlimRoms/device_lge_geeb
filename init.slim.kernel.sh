...
echo "----------------------------------------------------" >$log_file
echo "SlimGeeb Kernel - execution of kernel options init script" >>$log_file
echo "----------------------------------------------------" >>$log_file
echo "Kernel version : `uname -a`" >>$log_file

echo `date +"%F %R:%S : Checking for SlimGeeb kernel..."` >>$log_file

if [ "`uname -r | grep SlimGeeb`" == "" ]
  then
    echo `date +"%F %R:%S : No SlimGeeb kernel found, skip executing the config file"` >>$log_file
    exit
fi;

echo `date +"%F %R:%S : SlimGeeb kernel found, continue executing the config file..."` >>$log_file
